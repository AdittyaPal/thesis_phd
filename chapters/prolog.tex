...
Some additional technical details and discussions we have placed in appendices.
...
\newpage
tory in chemistry. In fact the term graph originates from the drawing, or graphing, of molecules [120]. In this thesis, we will turn our attention to the generation and analysis of chemical reaction networks. A reaction network describes
how chemical reactions allow molecules to interact within a chemistry [47].
These are used in a wide variety of research areas, such as the study of living organisms [115], drug synthesis and design [59], and the study of the chemistry
of planetary atmospheres [131]. Known chemical reaction networks are available through databases such as KEGG [81], containing metabolic networks,
or KIDA [125], containing gas-phase networks. The data provided by these
databases, however, is inherently subject to sampling bias; reactions are often
only included if they involve a number of molecules of interest in the chemistry
being examined [17]. This immediately becomes a limitation when modeling
reaction networks outside the scope of available databases or when seeking to
answer questions about general properties of reaction networks. To tackle such
problems, we must consider methods and formalisms applied to an artificial
chemistry. The main challenge in formalizing an artificial chemistry is found
in the potentially countless possible interactions between molecules. Given a
set of molecules, a reaction can not only create more of the given molecules,
but also produce novel new ones leading to a combinatorial explosion. Several
attempts over the years have been made at formalizing artificial chemistries
[39]. For example, in [15] the compounds of a reaction network are modeled
as binary strings, where two strings can interact to form a third binary string.
In [53], reaction networks are modeled using $\lambda$-calculus; compounds are $\lambda$ functions that can in turn interact to create new $\lambda$-functions. These methods
treat molecules as abstract entities. As a consequence, a molecules molecular
structure cannot be modeled, making it difficult to define molecular properties,
such as Gibbs free energy, that can be estimated from such information [80].
On the other hand, computing the three-dimensional structure of a molecule
is computationally expensive [65]. Modeling molecules as graphs provide an
excellent compromise between the retention of molecular structure and the
avoidance of computational complexity. While graphs have been used in chemistry for more than a hundred years [120], it was only recently shown in [17]
how to use graphs to model reaction networks of a chemistry. Here, molecules
are represented by graphs and reaction mechanisms, i.e. the sequence of elementary reactions that cause a chemical change, are modeled as graph transformation rules. Variations this technique was later examined in [130] and a
generic framework was implemented in [99]. Several tools have recently been
developed that use graph transformations to model chemistry or biochemistry
such as MØD [7], Kappa [23], and BioNetGen [20]. Our contributions for
this section will build upon the formalisms established and used by MØD to
model reaction networks. For this reason, we will in the following, give a brief

\newpage
dicated by the heuristic. Then recursively continue on the generated smaller
molecules until sufficiently simple or commercially available starting materials have been found. As an alternative to using heuristics trying to mimic
...
based on the existence of substructures of the target molecule that are isomorphic to easily accessible or available molecules found in libraries [Wipke
...
A major drawback of retrosynthesis, however, is that it is a greedy approach. In the attempt to make good choices during the retrosynthetic topdown recursion, it leaves out synthesis plans with costly last steps but much
...
on graph based descriptions of synthesis plans, and on formal quality measures of these plans. Hendrickson [Hendrickson 1977] models synthesis plans
...
it is computationally costly even for synthesis plans of depth three [Hendrickson 2002].
