%************************************************
\chapter{Generation of Reaction Networks}
\label{ch:preliminaries}
%************************************************

{\sffamily\slshape The text in this chapter is identical to Appendix A and B of the published version of the preprint at \href{https://arxiv.org/abs/2504.10609}{arXiv:2504.10609}.\\}

This chapter presents an alternative exposition, aiming to be less formal than Chapter~\ref{ch:toolbox}, of how reaction networks can be represented as hypergraphs and how pathways in them can be represented as integer hyperflows.

\section{Reactions as Hyperedges}
...
Some of the reactions that might occur when molecules of $v_0$ and $v_2$ are allowed to react is shown in Table~\ref{tab:sample_reactions}. To a chemist, not all the reactions would look feasible due to reactivity constraints on the molecules, which would disallow some of the reactions. However, all these reactions might possibly occur in the system in parallel, making it an example of a concurrent system. 
We would represent each reaction by a separate hyperedge and each class of molecules by a distinct vertex. This results in the structure shown in Figure~\ref{fig:sample_crnetwork}.
The reverse reactions are omitted from the hypergraph in Figure~\ref{fig:sample_crnetwork} to reduce clutter. However, in our actual modeling, we do add separate hyperedges to represent the forward and backward directions of reversible reactions.

This data structure representing the concurrent set of reactions in the system is called a multi-directed hypergraph or for simplicity, just a hypergraph and is shown in Figure~\ref{fig:sample_crnetwork}. An advantage of using this data structure is that is faithfully represents the many-to-many mappings which are present in reactions which are not unimolecular. Contrast this with a simple directed graph containing edges which map one vertex to another vertex, which are often used to represent and study networks in many situations. The hypergraph in Figure~\ref{fig: hypergraph} is an abstraction of another system of concurrent reactions.

Besides hypergraphs, reaction networks can also be represented by bipartite graphs~\cite{bipartite_crn,bipartite_crn2}, where the edges are represented by vertices instead of hyperedges. Therefore, in this representation there are two types of vertices: one representing molecules and the other reactions. While this allows the use of standard graph algorithms to the data structure, a single reaction consists of multiple edges. It becomes a subgraph in this representation instead of a single object. A particular variant representing a reaction network as a bipartite graph is the species-reaction graph~\cite{species_reaction_graph}, which uses two graphs: a species graph where the molecules are vertices and each reaction induces several edges, and a reaction graph where the reactions are vertices and an edge indicates that the product of the reaction is the reactant of the reaction on which the edge is incident. While these are simpler representations which are easier to analyse, it leads to loss of information compared to representing reaction networks as hypergraphs. Reaction networks can also be represented by a single stonchiometric matrix as it is often done during flux-balance and elementary mode analysis.
...
\begin{table}[!h]
	...
	\end{tiny}
	\end{center}
	\caption{Some reactions that might occur when the molecules $v_0$ and $v_1$ are allowed to react.}
    \label{tab:sample_reactions}
\end{table}
\clearpage
...
\begin{figure}[!h]
	...
	\end{subfigure}
	\vspace{0.5cm}
    \caption{The hyperflow denoting the pathway from Figure~\ref{tab:sample_path1} is shown in the hypergraph from Figure~\ref{fig:sample_crnetwork}.}
...
\section{Pathways as Hyperflows}
...
For example, in the list of reaction depicted in the hypergraph~\ref{fig:sample_crnetwork}:
\begin{itemize}
...
\section{Graph transformation rules}
...
\noindent The graph transformation rule for this class of reactions would be:
\begin{center}
...
\noindent The amide-iminol conversion is a more substantial example.
\begin{center}
\begin{tiny}
\textcolor{Blue}{
\begin{tabular}{ccc}
...
\noindent For a keto-enol tautomerism like
\begin{center}
\begin{tiny}
\textcolor{Blue}{
\begin{tabular}{ccc}
...
\noindent encodes the addition of a water or ammonia molecule to a carbonyl or imine group.
\begin{center}
\begin{tiny}
\textcolor{Blue}{
\begin{tabular}{ccc}
...
\section{Rule application to a graph}
\label{rule_application}

There are multiple ways to transform graphs and for the generation of reactions, the Double Pushout (DPO) method is used. As illustrative example is shown in Figure~\ref{fig:dpo_diagram}. The graph transformation operation can be labelled allowing changes to the labels of vertices or edges in the molecular graph to be specified by the rule. For chemical reactions, the graphs $G$ or $H$ might not be a single molecule (or in other words be a single connected component). 

\begin{figure}[!b]
    \centering
    \includegraphics[scale=1.0]{graphics/chapter1/dpo.pdf}
    \vspace{0.5cm}
    \caption{The diagram representing the graph transformation opreration $G\implies H$ using the Double Pushout method using the graph transformation rule $p\equiv L\leftarrow K\rightarrow R$ and the match $m_0$.}
    \label{fig:dpo_diagram}
\end{figure}
...
\begin{figure}[!th]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[scale=1.0]{graphics/chapter1/rule_app1.pdf}
        \caption{An example of the application of a graph transformation rule to one molecular graph.}
        \label{fig:rule_application1a}
    \end{subfigure}
    \vspace{0.4cm}\\
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[scale=1.0]{graphics/chapter1/rule_app2.pdf}
        \caption{An example of the application of a graph transformation rule to two molecular graphs. The left graph in the rule (top-span) has two connected components which be a subgraph in the disjoint union of two molecular graphs or one molecular graph.}
        \label{fig:rule_application1b}
    \end{subfigure}
...
When a graph transformation rule is applied to a set of reactant molecules, a reaction might occur. In the double pushout (DPO) approach, a rule is defined by three graphs, $(L \xleftarrow{} K \xrightarrow{} R)$, where $L$ is the left-hand side (the reaction pattern before the transformation), $R$ is the right-hand side (the reaction pattern after the transformation), and $K$ is the context graph containing the atoms and bonds that are preserved during the reaction and are necessary for it to occur.

To apply a rule, the algorithm first determines whether the left graph $L$ can be embedded into the graph $G$ representing the set of reactant molecules. This embedding, known as a graph morphism or match, identifies a subgraph of $G$ that satisfies the structural constraints of the rule. If a match is found, the matched subgraph is transformed by removing the atoms and bonds specified by $L \setminus K$ and inserting those specified by $R \setminus K$. The resulting graph, denoted by $H$, represents the set of product molecules generated by the reaction. This transformation preserves the connectivity of the unaffected parts of the reactant molecules and ensures that atoms are neither arbitrarily created nor destroyed, reflecting the mass conservation principles of chemical reactions. This process is illustrated in Figure~\ref{fig:rule_application1}. Note that in Figure~\ref{fig:rule_application1b}, there are two graphs on the reactant side to which the left side of the transformation rule has matched. The context graph $K$ in the transformation rule and the corresponding molecular graph is omitted from Figure~\ref{fig:rule_application1} for clarity, but it is important for correct order of the operations while transforming graphs: first edges are removed and then edges are added.

If the left-hand side of the rule, $L$, is not present in the set of molecules $G$, then that reaction template cannot be applied to the given set of molecules as shown in Figure~\ref{fig:rule_application2}.
...
\section{Construction of the reaction network}
...
\begin{figure}
    \begin{subfigure}{\textwidth}
        ...
        \caption{The reaction network (with $4$ molecules and $2$ reactions) after one recursive application of the used graph transformation rules.}
    \end{subfigure}
    \vspace{1.0cm}
    \\
    \begin{subfigure}{\textwidth}
        ...
        \caption{The reaction network (with $9$ molecules and $10$ reactions) after two recursive applications of the graph transformation rules.}
        \label{fig: recurse2}
    \end{subfigure}
    \vspace{0.5cm}
    \caption{The reaction network is constructed with the application of the graph transformation rules to the molecules already in the reaction network, which leads to the addition of new reactions (and possibly also new molecules) to the reaction network at each step. Repeating this multiple times expands the molecular space. In the representations of the reaction networks above, the part of the network already generated in the previous recursion has been shaded.}
...
To illustrate the construction of a reaction network by the recursive application of graph transformation rules to an input set of molecules, the graph transformation rules from Figure~\ref{fig:graphTransformationRules} in Chapter~\ref{ch:casestudy2} is applied to the input molecules water and 2-hydroxyethanenitrile is shown in Figures~\ref{fig: grow_network}, \ref{fig: recurse3} and \ref{fig: recurse4}.

Notice that some molecules in Figure~\ref{fig: grow_network}, such as the three-cycles, are not realistic. One can disallow their addition to the generated reaction network by using so-called right predicates. 
Right predicates filter out certain reactions from the network based on the properties of the molecular graphs that appear on the right-hand side of a derivation. Thus, it prunes the generated network and makes its growth more manageable and potentially more realistic. 
In this manner, the generated molecular space can be constrained not to have some specified subgraphs such as three cycles, four cycles, consecutive double bonds and others which are not commonly found in molecules.
Moreover, it can also be used to limit the total number of atoms or the number of atoms of a specific type in the generated molecules. For example, monosaccharide carbohydrate molecules with more than eight carbon atoms are rarely found in natural reaction systems, and hence can also be excluded from the generated network using an appropriate right predicate.
The effect of using the right predicate can be seen in the hypergraph in Figure~\ref{fig: recurse3}, which has $9$ molecules after three recursive expansion steps, while the hypergraph in Figure~\ref{fig: recurse2} already had $9$ molecules after two recursive steps when the right predicate was not used.
All the molecular graphs generated with three cycles in Figure~\ref{fig: recurse2} was not used to expand the network further to generate the reaction network illustrated in Figure~\ref{fig: recurse3}. 

There also exists a corresponding left predicate where certain molecular graphs can be prevented from reacting with other molecules and are constrainted to appear only as products of reactions. Reaction networks involving unreactive molecules (such as carbon dioxide) can be modeled realistically as once produced, they do not generally react further. Therefore, left predicates disallow reactions with such molecules as reactants.
...
The manner in which the graph transformation rules are used to generate the molecular space is called the exploration strategy. The rules can be applied sequentially, in parallel, in a repetitive manner for a specified number of iterations or in a particular composition defied by the user. 
Right predicates, described above, can also be included in the exploration strategy.
This results in a function which takes a set of molecular graphs as input and returns the hypergraph representing the generated reaction network. 
The input set of molecular graphs and the graph transformation rules define a graph grammar. Along with an exploration strategy, a graph grammar defines a molecular space recorded as a hypergraph without defining each constituent molecule and reaction explicitly.

While this Chapter describes the rule-based generation of reaction networks, in no way is it necessary to have a rule-based description of the network to apply the methods described in Chapters~\ref{ch:thermodynamic_model} and~\ref{ch:refined_model}. The methods from the latter chapters can also be applied to networks loaded from a database or constructed by explicit addition of each reaction as a hyperedge to form the hypergraph recording the molecular space.
