...
{\sffamily\slshape This chapter is identical to Section 4 of the published version of the preprint at \href{https://arxiv.org/abs/2411.15900}{arXiv:2411.15900}.\\}
...
\graffito{The atoms represented as vertices, labeled by the symbol of the element it represents, such as $C$ for a carbon atom, $H$ for a hydrogen atom, $N$ for a nitrogen atom, $O$ for an oxygen atom and so on. The bonds are represented as edges, labeled by bond types such as a single, double, triple or aromatic bond. The default bondtype is single.}
...
\begin{figure}[b]
    \includegraphics[scale=1.6]{graphics/chapter3/molGraphs.pdf}
    \vspace{0.2cm}
...
\begin{figure}[!t]
    \centering
    \includegraphics[width=1.2\textwidth]{graphics/chapter3/thermOracle.pdf}
    \vspace{0.5cm}
...
\begin{figure}[t]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=\textwidth]{graphics/chapter3/pathway1.pdf}
    \vspace{1.0cm}
    \caption{The pathway reported in \cite{implemented_CRN} with the sum of free energies $-178.412$ kJ mol$^{-1}$. This pathway requires four hyperedges (denoted by the squares, with the assigned hyperflow the integer in the square). The vertices in the pathway correspond to molecules, whose structures are listed below the pathway along with the label of the vertex.}
...
The pathway requires an inflow of three molecules of $v_0$ (hydrogen cyanide) and three molecules of $v_1$ (water) to produce the target molecule $v_5$. The chemical potential $x_v^{\Delta G}$ and the log concentration $x_v^K$ values assigned to each molecule by the MILP solver is listed in Table~\ref{tab:convSoln_molecules}, along with their molecular graphs. 
...
\begin{table}[!h]
    ...
    \end{tabular}
    \vspace{0.5cm}
    \caption{The MILP solver assigns the inflow for the allowed source molecules to generate an outflow of the queried target molecule, along with assigning the chemical potentials, $x_v^{\Delta G}$, and log concentration, $x_v^K$, values of the molecules in the pathway shown in Figure~\ref{fig: conventional_path}. The $x_v^{\Delta G}$ values are in atomic units (in Hartrees as returned by xTB) while the $x_v^K$ values are the dimensionless equivalent of $\log_{10} \frac{[\text{conc.}]}{1 \text{ mol lit}^{-1}}$, where conc. is the concentration of the molecule in mol lit$^{-1}$.}
...
\begin{table}[!h]
    ...
    \end{tabular}
    \vspace{0.5cm}
    \caption{The MILP solution assigned hyperflows $f_e$ to the hyperedges in the pathway, indicating the number of times they are used to effect the pathway shown in Figure~\ref{fig: conventional_path}. The set of vertices which is the inset of the hyperedge (or the reactants in the corresponding transition) is listed under in($v$), while the set of vertices which is the outset of the hyperedge (or the products in the corresponding transition) is listed under out($v$). $\overline{x}^{\Delta G}_e$ gives the free energy difference assigned to the hyperedge in kJ mol$^{-1}$.}
...
\begin{table}[!h]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \caption{The MILP solver assigns the inflow for the allowed source molecules to generate an outflow of the queried target molecule, along with assigning the chemical potentials, $x_v^{\Delta G}$, and log concentration, $x_v^K$, values of the molecules in the pathway shown in Figure~\ref{fig: shortest_path}. The units are the same as in Table~\ref{tab:convSoln_reactions}.}
    \label{tab:convSoln_molecules}
\end{table}
...
\begin{table}[!h]
    ...
    \end{tabular}
    \vspace{0.2cm}
    \caption{The MILP solution assigned hyperflows $f_e$ to the hyperedges in the pathway, indicating the number of times they are used to effect the pathway shown  in Figure~\ref{fig: shortest_path}. The set of vertices which is the inset of the hyperedge (or the reactants in the corresponding transition) is listed under in($v$), while the set of vertices which is the outset of the hyperedge (or the products in the corresponding transition) is listed under out($v$). $\overline{x}^{\Delta G}_e$ gives the free energy difference assigned to the hyperedge in kJ mol$^{-1}$.}
...
\begin{figure}[t]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=\textwidth]{graphics/chapter3/pathway2.pdf}
    \vspace{1.0cm}
    \caption{The pathway scoring the best when the model proposed in Chapter~\ref{ch:thermodynamic_model} was applied to the generated reaction network along with the objective function (8) with the simple sum of free energies $-313.718$ kJ mol$^{-1}$. This pathway requires four hyperedges (denoted by the squares, with the assigned hyperflow the integer in the square). The vertices in the pathway correspond to molecules, whose structures are listed below the pathway along with the label of the vertex.}
...
\begin{figure}[t]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=\textwidth]{graphics/chapter3/symAdd.pdf}
    \vspace{0.5cm}
...
This involved stepwise addition of of a HCN molecules followed by a H$_2$O molecules instead of direct addition of a foramide molecule. 

\begin{figure}[t]
    \centering
    \vspace{0.5cm}
    \includegraphics[width=\textwidth]{graphics/chapter3/stepAdd.pdf}
    \vspace{0.5cm}
...
\begin{figure}[!h]
    \centering
    \includegraphics[width=\textwidth]{graphics/chapter3/rxnList.pdf}
    \vspace{0.5cm}
