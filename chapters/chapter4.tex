%************************************************
\chapter{Pathways using Energy Barriers}
\label{ch:refined_model} %
%************************************************

{\sffamily\slshape The text in this chapter is identical to Section 3 of the published version of the preprint at \href{https://arxiv.org/abs/2504.10609}{arXiv:2504.10609} while a number of additional figures were added.\\}
...
A suitably crafted objective function should map a pathway to a real number (that can be compared objectively) to score the pathway in an appropriate manner. In this case, we would prefer pathways which have a high overall rate of occurring, and the pathway optimization problem formulated later would attempt o find a pathway with the maximum score when the pathway is evaluated using the objective function. Crafting an objective function often depends on the formulation of the problem and the user's preferences. 
...
In this case, we are interested in pathways from the given source molecules to the desired target molecule(s) that have a high overall probability of occurring in practice. This might not always be easy to determine when there are multiple pathways to the target molecule(s) as shown in Figure~\ref{fig: objFuncCases}. Considering individual reactions in a pathway as independent events (and abusing the notation~$e$ to mean the event of reaction~$e$ happening), the probability~$p_{\text{total}}$ of the entire set of reactions in the pathway occurring in one unit of time and one unit of volume is
\begin{align*}
...
\begin{figure}[t]
	\vspace{1cm}
    \centering
    \includegraphics[scale=1.1]{graphics/chapter4/null_cycle.pdf}
    \vspace{0.5cm}
...
Available ILP generally return the most optimal solution only. Some ILP solvers provide so-called solution pools which allow enumeration of solutions, but the process of populating this pool of solutions is often opaque.
...
A search tree is constructed by progressively restricting the domain of the variables. The sub-problem represented by each vertex is solved, and if it is infeasible, the entire sub-tree is pruned. For problems maximizing an objective function, the vertices in the constructed search tree is explored in the order of decreasing objective function value. This represent the use of a branch and bound styled process to enumerate multiple solution to the problem.

In this case, the specification lists exactly the variables $z_e$ for each $e\in E$ (indicating if the edge is used), and two pathways using the same set of hyperedges (but potentially with a difference in the flow value on some edges) are considered equal,
...
\begin{figure}[!t]
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[scale=1.1]{graphics/chapter4/subset1.pdf}
        \vspace{0.25cm}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[scale=1.1]{graphics/chapter4/subset2.pdf}
        \vspace{0.25cm}
    ...
    \begin{subfigure}{\textwidth}
        \centering
        \includegraphics[scale=1.1]{graphics/chapter4/overlap.pdf}
        \vspace{0.25cm}
...
In Figure~\ref{fig: cover}, the hyperflow of the entire figure (with the part with lower opacity) has a subset of hyperedges which form a solution by itself. Therefore, $\bigwedge z_e$ evaluates to \texttt{TRUE} when evaluated over the set of hyperedges in the solution depicted in deep blue. Also, the left-hand side of the constraint~\eqref{subset_elimination} is $\sum z_e = 4$ when evaluated over all the hyperedges in the hyperflow solution in dark blue which is $\nleq 4-1 = 3$. As a result, the hyperflow is not enumerated as a subsequent solution after the one in deep blue is added to the solution set.
Similarly the solution in Figure~\ref{fig:overlap}, would be excluded after either of the solutions in Figure~\ref{fig:solution1} or~\ref{fig:solution2} has been added to the solution set. The left-hand side of the constraint~\eqref{subset_elimination} is $\sum z_e = 4$ when evaluated over all the hyperedges in the hyperflow in Figure~\ref{fig:solution1} which is $\nleq 4-1 = 3$. Therefore, the hyperflow in Figure~\ref{fig:overlap} is not enumerated as a subsequent solution after the one in Figure~\ref{fig:solution1} is added to the solution set. However, addition if the solution in Figure~\ref{fig:solution1} does not exclude the solution in~\ref{fig:solution2} as the left-side of constraint~\eqref{subset_elimination} evaluates to $\sum z_e = 3$ (when summed over all hyperedges in the solution in Figure~\ref{fig:solution1}) which is $\leq 4-1 = 3$.
...
\begin{figure}[t]
	\vspace{0.8cm}
    \centering
    \includegraphics[scale=1.1]{graphics/chapter4/selfLoop.pdf}
    \vspace{0.5cm}
...
\begin{figure}[!h]
    ...
    \begin{subfigure}[b]{0.33\textwidth}
        \centering
        \includegraphics[scale=1.1]{graphics/chapter4/pathway_tvAssign.pdf}
    \end{subfigure}%
    \hspace{1cm}
