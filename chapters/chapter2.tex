...
Since we are extending the existing integer hyperflow formulation defined earlier, the base ILP formulation for modeling flow conservation in pathways in reaction networks stated by Equation~\eqref{int_hyperflow_jakob} is inherited from that work.
\[
\forall v\in V\colon \sum_{ e\in \overline{E} } s_{ve}^+ f_e - \sum_{ e\in \overline{E} } s_{ve}^- f_e = 0
\]

The working of Constraint~\eqref{int_hyperflow_jakob} is illustrated in the four example cases in Figure~\ref{fig: flowConv}. 
